...
class WalletRepository(
    ...
    suspend fun updateWallet(wallet: WalletEntity) = withContext(Dispatchers.IO) {
        ...
        if (userDao != null) {
            ...
            if (user != null) {
                val updatedCoins = wallet.currentCoins
                val updatedBalance = wallet.currentCoins * com.example.data.config.AdConfig.COINS_TO_USD_RATIO
                userDao.updateCoins(updatedCoins, updatedBalance)
                val refreshedUser = userDao.getActiveUserProfile()
                if (refreshedUser != null) {
                    com.example.data.firebase.FirebaseManager.syncUserProfile(refreshedUser)
                }
            }
        }
    }
    ...
    suspend fun insertRedeemRecord(redeem: RedeemHistoryEntity) = withContext(Dispatchers.IO) {
        ...
        com.example.data.firebase.FirebaseManager.syncRedeemRecord(redeem.userId, redeem)
    }
    ...
    suspend fun insertWithdrawalRecord(withdrawal: WithdrawalHistoryEntity) = withContext(Dispatchers.IO) {
        ...
        com.example.data.firebase.FirebaseManager.syncWithdrawalRecord(withdrawal.userId, withdrawal)
    }
    ...
    suspend fun insertActivityRecord(activity: WalletActivityEntity) = withContext(Dispatchers.IO) {
        ...
        com.example.data.firebase.FirebaseManager.syncWalletActivity(activity.userId, activity)
    }
