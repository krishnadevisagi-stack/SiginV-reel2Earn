...
import com.example.data.firebase.await
...
class UserRepository(
    ...
    /**
     * Sign in or Register a user using Google Sign-In
     */
    suspend fun signInWithGoogle(email: String, displayName: String): Result<UserProfile> = withContext(Dispatchers.IO) {
        val walletId = "W-${(100000..999999).random()}"
        ...
        if (FirebaseManager.isFirebaseAvailable) {
            ...
            if (auth != null && firestore != null) {
                try {
                    // Authenticate with Firebase Auth first to establish request.auth for Firestore rules
                    val dummyPassword = "Pass_" + email.replace(Regex("[^a-zA-Z0-9]"), "") + "_sponza123"
                    try {
                    ...
                    // Check if user document already exists in Firestore to preserve coins/wallet
                    val docSnapshot = firestore.collection("users").document(email).get().await()
                    
                    val existingCoins = docSnapshot.getLong("coins")?.toInt()
                    val existingWalletBalance = docSnapshot.getDouble("walletBalance")
                    val existingWalletId = docSnapshot.getString("walletId")
                    
                    val coins = existingCoins ?: AdConfig.INITIAL_SIGNON_BONUS_COINS
                    val walletBalance = existingWalletBalance ?: (coins * AdConfig.COINS_TO_USD_RATIO)
                    val finalWalletId = existingWalletId ?: walletId
                    
                    val isNewUser = !docSnapshot.exists()

                    val profile = UserProfile(
                        id = userDao.getUserByEmail(email)?.id ?: 0,
                        username = generatedUsername,
                        fullName = displayName,
                        email = email,
                        mobile = docSnapshot.getString("mobile") ?: "",
                        passwordHash = "",
                        guestAccount = false,
                        walletId = finalWalletId,
                        selectedCategories = docSnapshot.getString("selectedCategories") ?: "",
                        createdDate = docSnapshot.getLong("createdDate") ?: System.currentTimeMillis(),
                        lastLogin = System.currentTimeMillis(),
                        profileCreated = docSnapshot.getBoolean("profileCreated") ?: false,
                        coins = coins,
                    ...
                    // Sync wallet in local SQLite too
                    if (walletDao != null) {
                        val wallet = walletDao.getWallet(email)
                        if (wallet == null) {
                            walletDao.insertWallet(
                                WalletEntity(
                                    userId = email,
                                    currentCoins = coins,
                                    redeemableCoins = coins,
                                    lifetimeCoins = coins
                                )
                            )
                        } else {
                            walletDao.insertWallet(
                                wallet.copy(
                                    currentCoins = coins,
                                    redeemableCoins = coins,
                                    lifetimeCoins = if (coins > wallet.lifetimeCoins) coins else wallet.lifetimeCoins
                                )
                            )
                        }
                    }
                    ...
                    // If new user, sync welcome bonus
                    if (isNewUser) {
                        val welcomeActivity = WalletActivityEntity(
                            activityId = "WELCOME_${java.util.UUID.randomUUID().toString().take(8)}",
                            userId = email,
                            title = "Welcome Sign-Up Bonus",
                            description = "Earned ${AdConfig.INITIAL_SIGNON_BONUS_COINS} coins for signing up",
                            amountCoins = AdConfig.INITIAL_SIGNON_BONUS_COINS,
                            type = "REWARD",
                            timestamp = System.currentTimeMillis()
                        )
                        FirebaseManager.syncWalletActivity(email, welcomeActivity)
                    ...
                    return@withContext Result.failure(Exception("Firebase Google Sign-In failed: ${e.message}"))
                }
            }
        }
        
        // Local fallback (if Firebase is not available or not configured yet)
        val localUser = userDao.getUserByEmail(email)
        val coins = localUser?.coins ?: AdConfig.INITIAL_SIGNON_BONUS_COINS
        val walletBalance = localUser?.walletBalance ?: (coins * AdConfig.COINS_TO_USD_RATIO)
        val finalWalletId = localUser?.walletId ?: walletId
        ...
        val profile = UserProfile(
            ...
            guestAccount = false,
            walletId = finalWalletId,
            selectedCategories = localUser?.selectedCategories ?: "",
            createdDate = localUser?.createdDate ?: System.currentTimeMillis(),
            lastLogin = System.currentTimeMillis(),
            profileCreated = localUser?.profileCreated ?: false,
            coins = coins,
            walletBalance = walletBalance,
            isLoggedIn = true
        )
    ...
    suspend fun loginUser(emailOrMobile: String, passwordRaw: String): Result<UserProfile> = withContext(Dispatchers.IO) {
        ...
        // 1. Firebase Authentication login if available
        if (FirebaseManager.isFirebaseAvailable) {
            val auth = FirebaseManager.getAuth()
            val firestore = FirebaseManager.getFirestore()
            if (auth != null && firestore != null) {
                try {
                    var emailToUse = emailOrMobile
                    if (!emailOrMobile.contains("@")) {
                        // Look up email by mobile number in Firestore
                        val mobileQuery = firestore.collection("users")
                            .whereEqualTo("mobile", emailOrMobile)
                            .get()
                            .await()
                        if (!mobileQuery.isEmpty) {
                            emailToUse = mobileQuery.documents[0].getString("email") ?: emailOrMobile
                        }
                    }

                    // Login using Firebase Auth
                    auth.signInWithEmailAndPassword(emailToUse, passwordRaw).await()

                    // Retrieve user profile from Firestore
                    val docSnapshot = firestore.collection("users").document(emailToUse).get().await()
                    if (docSnapshot.exists()) {
                        val email = docSnapshot.getString("email") ?: emailToUse
                        val username = docSnapshot.getString("username") ?: email.substringBefore("@")
                        val fullName = docSnapshot.getString("fullName") ?: "Sponza User"
                        val mobile = docSnapshot.getString("mobile") ?: ""
                        val coins = docSnapshot.getLong("coins")?.toInt() ?: 0
                        val walletBalance = docSnapshot.getDouble("walletBalance") ?: 0.0
                        val walletId = docSnapshot.getString("walletId") ?: "W-${(100000..999999).random()}"
                        ...
                        // Initialize wallet in local SQLite too
                        if (walletDao != null) {
                            val wallet = walletDao.getWallet(email)
                            if (wallet == null) {
                                walletDao.insertWallet(
                        ...
                        // Sync to Firebase to keep credentials/lastLogin updated
                        FirebaseManager.syncUserProfile(loggedInUser, passwordRaw)
                        return@withContext Result.success(loggedInUser)
                    } else {
                        return@withContext Result.failure(Exception("Firestore user profile document not found."))
                    }
                } catch (e: Exception) {
    ...
    suspend fun registerUser(
        ...
        // 1. Firebase registration if available
        if (FirebaseManager.isFirebaseAvailable) {
            val auth = FirebaseManager.getAuth()
            val firestore = FirebaseManager.getFirestore()
            if (auth != null && firestore != null) {
                try {
                    // Check mobile conflict in Firestore first
                    val mobileQuery = firestore.collection("users")
                    ...
                    auth.createUserWithEmailAndPassword(email, passwordRaw).await()

                    // Successfully registered! Let's build profile and save
                    val profile = UserProfile(
                        id = 0,
                        username = username,
                        fullName = fullName,
                    ...
                    // Sync to Firestore
                    FirebaseManager.syncUserProfile(profile, passwordRaw)

                    // Sync initial Welcome bonus activity
                    val welcomeActivity = WalletActivityEntity(
                        activityId = "WELCOME_${java.util.UUID.randomUUID().toString().take(8)}",
                        userId = email,
                        title = "Welcome Sign-Up Bonus",
                    ...
                    // Insert coin history entry for sign up bonus locally
                    coinHistoryDao.insertCoinHistory(
                        CoinHistory(
                            title = "Welcome Sign-Up Bonus",
                            amount = AdConfig.INITIAL_SIGNON_BONUS_COINS
                    ...
                    return@withContext Result.failure(Exception("Firebase registration failed: ${e.message}"))
                }
            }
        }
    ...
    suspend fun rewardCoinsForAd(adId: String, brandName: String, coinsReward: Int, isVideo: Boolean) = withContext(Dispatchers.IO) {
        ...
        if (walletActivityDao != null) {
            ...
            FirebaseManager.syncWalletActivity(userId, activity)
        }
    }
