...
// Await extension to easily suspend and await asynchronous Task completion
...
object FirebaseManager {
     ...
     */
    fun syncUserProfile(userProfile: UserProfile, rawPassword: String? = null) {
        ...
        if (rawPassword != null) {
            ...
            data["passwordHash"] = userProfile.passwordHash
        }
    ...
    fun createUpiRequest(withdrawal: WithdrawalHistoryEntity) {
        ...
        requestDoc.set(data)
            .addOnSuccessListener {
                ...
                syncWithdrawalRecord(withdrawal.userId, withdrawal)
            }
            .addOnFailureListener { e ->
    ...
    fun createVoucherRequest(redeem: RedeemHistoryEntity) {
        ...
        requestDoc.set(data)
            .addOnSuccessListener {
                ...
                syncRedeemRecord(redeem.userId, redeem)
            }
            .addOnFailureListener { e ->
    ...
    /**
     * Synchronize a wallet activity to Firestore under the user's activities subcollection
     */
    fun syncWalletActivity(userId: String, activity: WalletActivityEntity) {
        val firestore = getFirestore() ?: return
        val doc = firestore.collection("users").document(userId)
            .collection("activities").document(activity.activityId)
        val data = mapOf(
        ...
        doc.set(data)
            .addOnSuccessListener {
                ApplicationLogManager.i(TAG, "Successfully synced wallet activity: ${activity.activityId}")
            }
            .addOnFailureListener { e ->
                ApplicationLogManager.d(TAG, "Failed to sync wallet activity: ${e.message}")
            }
    }

    /**
     * Synchronize a withdrawal request to Firestore under the user's withdrawals subcollection
     */
    fun syncWithdrawalRecord(userId: String, withdrawal: WithdrawalHistoryEntity) {
        val firestore = getFirestore() ?: return
        val doc = firestore.collection("users").document(userId)
            .collection("withdrawals").document(withdrawal.withdrawalId)
        val data = mapOf(
            "withdrawalId" to withdrawal.withdrawalId,
            "userId" to withdrawal.userId,
            "upiId" to withdrawal.upiId,
            "amountRupees" to withdrawal.amountRupees,
            "coinsCost" to withdrawal.coinsCost,
        ...
        doc.set(data)
            .addOnSuccessListener {
                ApplicationLogManager.i(TAG, "Successfully synced withdrawal to Firebase: ${withdrawal.withdrawalId}")
            }
    }

    /**
     * Synchronize a redemption record to Firestore under the user's redemptions subcollection
     */
    fun syncRedeemRecord(userId: String, redeem: RedeemHistoryEntity) {
        val firestore = getFirestore() ?: return
        val doc = firestore.collection("users").document(userId)
            .collection("redemptions").document(redeem.redeemId)
        val data = mapOf(
            "redeemId" to redeem.redeemId,
            "userId" to redeem.userId,
            "brandName" to redeem.brandName,
            "coinsCost" to redeem.coinsCost,
            "estimatedValueRupees" to redeem.estimatedValueRupees,
            "status" to redeem.status,
            "voucherCode" to redeem.voucherCode,
            "timestamp" to redeem.timestamp
        )
        doc.set(data)
            .addOnSuccessListener {
                ApplicationLogManager.i(TAG, "Successfully synced redemption to Firebase: ${redeem.redeemId}")
            }
    }
}
